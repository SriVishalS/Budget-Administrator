...
function traduzGUI() {
  document.getElementById('exp').value = lang.menu.expense;
...
function gravaCategorias(click = true) {
  if (click) {
    if (
      document.getElementById("categoriainput").value.match(/^[0-9a-zA-Z]+$/)
    ) {
...
function recuperaCategorias() {
  ...
  for (let i = 0; i < cats.length; i++) {
    if (cats[i].length > 0) {
      ...
      document.getElementById("categorias").innerHTML +=
        "<div class='categoriarow'><label for='categoria_" +
        cats[i] +
        "'>" +
        "<input type='checkbox' name='categoria' id='categoria_" +
        cats[i] +
        "' " +
    ...
    for (i = id - 1; i > 0; i--) {
      if (localStorage.getItem(i)) {
        ...
        if (dados[1] != ultimadata) {
          ...
          document.getElementById("conteudodespesas").innerHTML +=
            "<div class='data'>" +
            dias[parseInt(dados[0])] +
            ", " +
            dados[1] +
            "<div id='" +
            dados[1] +
            "' class='valordia'></div></div>";
          valordia = 0;
        }

        valordia += parseFloat(dados[3]);
        valormes += parseFloat(dados[3]);
        document.getElementById(`conteudodespesas`).innerHTML += `<div class='despesa'><div id='despesacell1'>${dados[4]}</div>
	  <div id='despesacell2'>${
          dados[2]
        }</div><div id='despesacell3'>${money} ${parseFloat(dados[3]).toFixed(
          1
        )}</div><div id='despesacell4'><input type='checkbox' name='despesa' value='${i}' onClick='habilitaApagar("despesa","apagarbotao")'></div></div>`;
        document.getElementById("" + ultimomes + ultimoano).innerHTML =
          money + " " + valormes.toFixed(1);
        document.getElementById("" + dados[1]).innerHTML =
          money + " " + valordia.toFixed(1);
      }
    }
...
function predizCategoria() {
  if (_x && _x.length > 0) {
    ...
    timeout = setTimeout(function () {
      document.getElementById("local").selectedIndex = previsao([
        diasemana,
        hora,
        textInput.value,
      ]);
